...
function createCardHTML(item) {
    ...
    // Google'ın resmi daha iyi anlaması için Başlık + Açıklama birleştiriyoruz.
    // Örn: "Afyon Beyazı - Homojen yapısı ve zarif damarlarıyla klasik lüks."
    const seoAltText = `${item.title} - ${item.desc}`;
    ...
    if(item.material === 'traverten') badgeClass = "bg-amber-700 text-white"; // Traverten için de renk ekledim
...
function renderGallery(filterValue, filterGroup) {
    ...
    if (filteredItems.length > 0) {
        ...
        galleryGrid.innerHTML = `
            <div class="col-span-full text-center py-12 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50">
                <p class="text-gray-500 font-medium text-lg">Bu kategoride ürünler hazırlanıyor...</p>
                <p class="text-gray-400 text-sm">Diğer kategorilere göz atabilirsiniz.</p>
            </div>`;
    }
}
...
filterButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        // HTML'den veriyi al ve filtrele
